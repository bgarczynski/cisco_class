...
def get_portchannel_sum(sw1):
	...
	sh_chan_port_dict = xmltodict.parse(sh_chan[1])

	if sh_chan_dict['ins_api']['outputs']['output']['body']['TABLE_channel']['ROW_channel']['port-channel'] != None:
...
def main():
	...
	for d in devices_Crawled:
		...
		for portchan in d.portChannels:
			...
			#print d.neighbors
			#print d.portChannels
			#print "Port-Channel Number:",portchan['group']
			#print "Layer: ",portchan['layer']
			#print "Status: ",portchan['status']
			#portChanPort = sh_chan_port_dict['ins_api']['outputs']['output']['body']['TABLE_channel']['ROW_channel']['TABLE_member']['ROW_member']
			#print portchan.keys()
		
			if "TABLE_member" in portchan.keys() != None:
				print "Port-Channel Number:",portchan['group']
				print "Member Ports: ",portchan['TABLE_member']['ROW_member']['port']
				...
				#has_pchan = portchan['TABLE_member']['ROW_member']['port']
				#if has_pchan != None: 
					#print "Layer 2/3: ",portchan['layer']
					#print "Status: ",portchan['status']
				#print portchan

			#print "Member Port: ",portchan['port']
			#print "\n"
			#print "\t",d.hostname 
			#print "\t",d.neighbors
